# Plan — v1.1.2 (Thor Guard multi-feed, catalog & Admin control)

Status: **shipped as v1.1.2**.  
Related prior work: **v1.1.1** condition announce toggles, persisted `monitor`, defaults (60s poll, hours 09:00–16:00).

**Design mandate:** Thor Guard is a **life-safety** subsystem. v1.1.2 is not “add two backup URLs.” It is a full Admin-controlled multi-feed monitor with explicit failover policy, per-feed announcement control, timing, verification, and live operator visibility — every behavior an on-site admin can inspect and change without editing JSON by hand.
...
6. **At most one active feed** drives condition/lock at a time (no “worst of three” merge in v1.1.2 — explicit ordered failover).
...
```json
{
  ...
  "generated_for": "TARR Annunciator v1.1.2+",
  "sensors": [
    {
      "id": "FL0115",
      "path_prefix": "tp",
...
Feed-switch **audio is not part of failover policy**. See §5.6.
...
### 5.6 Feed-switch announcements (separate from failover policy)

**Requirement:** extremely precise Admin control — not one global “play something on any switch.”

Failover **policy** (triggers, counts, failback mode) stays announcement-free. Switch audio is a **separate** config list + Admin card.

Each rule can target a specific transition, e.g.:

- Primary → Failover 1 (secondary) → MP3 A  
- Primary → Failover 2 (tertiary) → MP3 B  
- Failover 1 → Primary (failback) → MP3 C  
- Optional: only when destination (or source) sensor is a given catalog id / displayname

```json
"feed_switch_announcements": [
  {
    "id": "p_to_f1",
    "enabled": false,
    "from_feed_id": "primary",
    "to_feed_id": "failover_1",
    "reason": "failover",
    "match_sensor_id": "",
    "match_displayname": "",
    "audio_file": "",
...
| Field | Meaning |
|-------|---------|
| `from_feed_id` / `to_feed_id` | `primary` \| `failover_1` \| `failover_2` \| `*` (any) |
| `reason` | `any` \| `failover` \| `failback` \| `pin` |
| `match_sensor_id` / `match_displayname` | Optional; empty = any. Matched against **destination** feed’s configured sensor/displayname (prefer config; live XML values as secondary check once known) |
| `enabled` | Default **false** — quiet until operator opts in |
| `audio_file` | Under `static/mp3/lightning/` |

**Resolution (highest specificity wins; one PA max per transition):**

1. Exact from+to+reason+sensor  
2. Exact from+to+sensor  
3. Exact from+to+reason  
4. Exact from+to  
5. Wildcards (`*`) with same sensor/reason specificity ladder  
6. No match → silent

Does not change Red Alert lock/condition; lightning-class queue so it is not suppressed mid-alert.
...
### C. Failover & failback policy (new card, border-warning) — policy only
...
- **No announcement / MP3 controls in this card**

### C2. Feed-switch announcements (new card, separate)

Precise optional PA when active feed id changes. Default: all rules disabled → **quiet**.

Editable rule table (add / remove / enable):

| Enabled | From | To | Reason | Match sensor (optional) | MP3 | Test |
|---------|------|----|--------|-------------------------|-----|------|
| ✓ | Primary | Failover 1 | failover | (any) | `switch_p_to_f1.mp3` | ▶ |
| ✓ | Primary | Failover 2 | failover | (any) | `switch_p_to_f2.mp3` | ▶ |
| ✓ | Failover 1 | Primary | failback | Tradewinds (FL0115) | `back_primary.mp3` | ▶ |

- From/To: `primary` / `failover_1` / `failover_2` / `*` (any)
- Reason: `any` / `failover` / `failback` / `pin`
- Match sensor: optional catalog/displayname filter on **destination** feed
- Supports distinct audio for primary→secondary vs primary→tertiary, and sensor-specific variants
- Resolution: highest specificity wins; one PA per transition; no match = silent
- Saved with monitor (`feed_switch_announcements`)
...
- [ ] Feed-switch PA matrix: quiet with no enabled rules; distinct MP3 for primary→failover_1 vs primary→failover_2; optional sensor match; failback/pin reasons; does not alter lock/condition
...
- Single global “one MP3 for every switch” as the only control (superseded by per-transition / per-sensor matrix)
- Stacking multiple switch PAs for one transition event
...
2. **Pin active feed for drills:** in v1.1.2 or follow-up? → **Recommend: include** (cheap, high drill value).
