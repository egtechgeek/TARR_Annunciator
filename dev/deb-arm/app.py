...
This version uses pygame + PulseAudio for audio and crontab for scheduling
"""
...
def play_audio(file_path):
    """Play audio file using pygame with PulseAudio volume control"""
    ...
    try:
        ...
        # Set pygame volume (convert from 0-100 to 0.0-1.0)
        pygame_volume = CURRENT_VOLUME / 100.0
...
def audio_status():
    ...
    return {
        'audio_available': AUDIO_AVAILABLE,
        'audio_backend': 'pygame + pulseaudio',
...
def cli_args():
    parser = argparse.ArgumentParser(description="TARR Annunciator - Raspberry Pi (Pygame + PulseAudio)")
